...
class User(db.Model):
    ...
    generated_content = db.relationship('GeneratedContent', backref='user', lazy='dynamic', cascade='all, delete-orphan')
...
class GeneratedContent(db.Model):
    """Track ALL content generated by AI (for analytics), whether saved or not"""
    __tablename__ = 'generated_content'
    
    id = db.Column(db.String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    user_id = db.Column(db.String(36), db.ForeignKey('users.id'), nullable=False, index=True)
    saved_content_id = db.Column(db.String(36), db.ForeignKey('content_items.id'), nullable=True, index=True)  # NULL if not saved
    ...
    # Status
    was_saved = db.Column(db.Boolean, default=False)  # True if user clicked "Save"
    was_improved = db.Column(db.Boolean, default=False)  # True if user clicked "Improve"
    was_copied = db.Column(db.Boolean, default=False)  # True if user clicked "Copy"
    ...
    def to_dict(self):
        return {
            'id': self.id,
            'user_id': self.user_id,
            'saved_content_id': self.saved_content_id,
            'content_type': self.content_type,
            'tone': self.tone,
...
class Analytics(db.Model):
    ...
    generated_content_id = db.Column(db.String(36), db.ForeignKey('generated_content.id'), nullable=True, index=True)
